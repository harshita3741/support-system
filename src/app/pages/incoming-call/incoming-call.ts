...
export class IncomingCall implements OnInit, OnDestroy {
  ...
  prescription = { diagnosis: '', advice: '', investigations: '', followUpDate: '', duration: '' };
  ...
  toggleChat() { this.chatOpen = !this.chatOpen; if (this.chatOpen) this.prescriptionOpen = false; }
  ...
  goBack() { this.cleanup(); this.router.navigate(['/queue']); }
  ...
  removeMedicine(index: number) { if (this.medicines.length > 1) this.medicines.splice(index, 1); }
  ...
  generatePDF(data: any, date: string) {
    ...
    try {
      ...
      meds.forEach((m: any) => {
        if (m.name) medsHtml += `<div style="margin:6px 0 2px 16px;font-size:14px">&bull; Tab. ${m.name} ${m.dosage || ''} ${m.frequency || ''} ${m.duration ? 'for ' + m.duration : ''}</div>`;
      });
    } catch { medsHtml = `<div style="margin-left:16px">${data.medicines}</div>`; }
    ...
    const html = `<html><head><title>Prescription</title><style>
      *{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;padding:48px 56px;max-width:720px;margin:auto;color:#111}
      .hosp{font-size:22px;font-weight:bold;text-align:center}.docname{font-size:16px;font-weight:bold;text-align:center;margin-top:4px}
      .center{text-align:center;font-size:13px;color:#444;line-height:1.6}.divider{border:none;border-top:2.5px solid #0d6e6e;margin:16px 0}
      .patient-block{font-size:14px;line-height:1.9;margin-bottom:4px}.rx-symbol{font-size:30px;font-style:italic;font-weight:bold;margin:20px 0 8px;font-family:Georgia,serif}
  ...
  getDoctorSession() { try { return JSON.parse(localStorage.getItem('doctor') || '{}'); } catch { return {}; } }
