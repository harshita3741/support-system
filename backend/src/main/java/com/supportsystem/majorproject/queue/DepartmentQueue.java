...
public class DepartmentQueue {
    ...
    public void addCase(MedicalCase medicalCase){
        ...
        switch (medicalCase.getDepartment()){
            ...
            case "CARDIO":
                cardioQueue.add(medicalCase);
                break;

            case "NEURO":
                neuroQueue.add(medicalCase);
                break;

            case "ORTHO":
                orthoQueue.add(medicalCase);
                break;
        }
    }

    public MedicalCase getCardioCase() throws InterruptedException {
        return cardioQueue.take();
    }

    public MedicalCase getNeuroCase() throws InterruptedException {
        return neuroQueue.take();
    }

    public MedicalCase getOrthoCase() throws InterruptedException {
        return orthoQueue.take();
    }
}
